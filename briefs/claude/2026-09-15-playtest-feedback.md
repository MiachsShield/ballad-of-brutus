...
3. Log **ALL** playtest feedback — delights and friction, not just the top
   issues — ordered by player impact, under your `## Feedback — Claude`
   section in `MEETING.md`.

Design-lane lens: does the build express the 12 rulings (tribute costs actions, Second Wind appears while stunned, affection-range takeovers, win = every region visibly under Brutus)? Flag anywhere the build contradicts the design docs.

## Rules
- Work on branch `claude/playtest-feedback` off current main. Edit only your Feedback

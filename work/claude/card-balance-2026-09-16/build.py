...
BUILD=os.path.join(HERE,'..','..','..','builds','brutus-1_0_a0mk-kavi-balanced.html')
...
# ---- per-card decisions: copies (0 = not in the default deck), category, frames, exposure, changes ----
# Access tiers (canon/decisions-2026-09-17.md #5): general = any deck; class = that class only;
# signature = restricted to a unique character, an ability, or gear. Signature cards never sit
# in a class default deck unless the deck belongs to that unique character (Brutus's own decks).
# category: offense | defense | heal | buff | debuff | finisher
P='priest_'; W='warrior_'
D = {
 # PRIEST ------------------------------------------------------------
 P+'quick_strike':      dict(copies=3, cat='offense'),
 P+'surge':             dict(copies=0, cat='offense'),
 P+'rigor':             dict(copies=3, cat='offense', frames=(180,260)),
 P+'cleave':            dict(copies=3, cat='offense'),
 P+'smite_priest':      dict(copies=3, cat='offense', frames=(380,560)),
 P+'blazewhirl':        dict(copies=0, cat='offense', frames=(420,620), exp=1.4),
 P+'heavy_blow':        dict(copies=2, cat='offense'),
 P+'radiant_cleave':    dict(copies=0, cat='offense'),
 P+'excommunication':   dict(copies=1, cat='finisher', frames=(1000,1150), exp=1.8,
 ...
 P+'bulwark_slam':      dict(copies=1, cat='defense', frames=(900,800), exp=1.5,
 ...
 P+'last_rite':         dict(copies=1, cat='heal', frames=(200,350)),
 ...
 P+'vanish_step':       dict(copies=0, cat='defense'),
 P+'anthem_of_the_unbroken': dict(copies=1, cat='defense', frames=(800,900), exp=1.6,
 ...
 P+'rite_of_the_held_wound': dict(copies=1, cat='defense'),
 ...
 P+'blessing':          dict(copies=0, cat='buff'),
 P+'benediction':       dict(copies=1, cat='buff', frames=(250,300)),
 P+'steady_hands':      dict(copies=1, cat='buff'),
 P+'sacrament_of_nerve':dict(copies=0, cat='buff'),
 P+'alms_taker_s_ear':  dict(copies=1, cat='buff'),
 ...
 P+'radiant_glare':     dict(copies=2, cat='debuff', frames=(420,600), exp=1.4),
 ...
 P+'vow_of_silence':    dict(copies=0, cat='debuff', note="Needs caster enemies (lich, vampire, imp, goblin shaman). Joins the deck once the bestiary tags land."),
 # WARRIOR -----------------------------------------------------------
 ...
 W+'killing_shout':     dict(copies=1, cat='finisher', frames=(1000,1150), exp=1.8,
 ...
 W+'kick_the_knee':     dict(copies=2, cat='debuff'),
 W+'sweeping_leg':      dict(copies=1, cat='debuff'),
 W+'feint_high':        dict(copies=2, cat='debuff'),
 W+'venom_strike':      dict(copies=0, cat='debuff'),
 W+'thunderclap':       dict(copies=0, cat='debuff', frames=(450,650), exp=1.4),
 W+'frostbite_slash':   dict(copies=0, cat='debuff'),
 W+'toxic_cloud':       dict(copies=0, cat='debuff'),
 W+'iron_jaw':          dict(copies=2, cat='defense'),
 W+'no_retreat':        dict(copies=2, cat='defense'),
 W+'rime_shell':        dict(copies=0, cat='defense'),
...
# Access tier per id. Anything not listed is 'class'.
GENERAL = {P+'quick_strike',P+'cleave',P+'heavy_blow',P+'weak_cure'}          # Brutus's base kit: allowed in every deck
SIGNATURE = {                                                               # restricted: who/what grants it
 P+'bulwark_slam':'unique: Brutus (Resolute stance)',
 P+'blazewhirl':'gear: Fire weapon', P+'radiant_cleave':'gear: Holy weapon', W+'thunderclap':'gear: Lightning weapon',
 W+'frostbite_slash':'gear: Frost weapon', W+'venom_strike':'gear: Poison weapon', W+'umbral_reaper':'gear: Shadow weapon',
 P+'surge':'gear: Lightning accessory', P+'blessing':'gear: Holy accessory', P+'vanish_step':'gear: Shadow accessory',
 W+'rime_shell':'gear: Frost accessory', W+'toxic_cloud':'gear: Poison accessory',
}
...
for rid,row in live.items():
    ...
    new['category']=d['cat']
    new['access']='general' if rid in GENERAL else ('signature' if rid in SIGNATURE else 'class')
    if rid in SIGNATURE: new['grantedBy']=SIGNATURE[rid]
...
for cls in ('Priest','Warrior'):
    ...
    sigs=[r['name'] for r in rows if r['access']=='signature' and r['deck_copies']]
    assert all(r['deck_copies']==1 for r in rows if r['category']=='finisher' and r['deck_copies'])
    # gear signatures come from equipment, never from the class default deck
    assert not [r['id'] for r in rows if r.get('grantedBy','').startswith('gear') and r['deck_copies']], 'gear card in class deck'
    print(cls,'deck',tot,'unique in deck',sum(1 for r in rows if r['deck_copies']),'not in deck',sum(1 for r in rows if not r['deck_copies']),dict(comp),'signatures in deck',sigs)
    dmgcards=sum(r['deck_copies'] for r in rows if r['dmg']>0 or r.get('mercyStrike'))
    plain=sum(r['deck_copies'] for r in rows if r['dmg']>0)
    print('  damage cards:',plain,'(+Mercy Strike:',dmgcards,') | no-damage 4-card hand: %.1f%% (%.1f%% with Mercy Strike live)'%(100*comb(40-plain,4)/comb(40,4),100*comb(40-dmgcards,4)/comb(40,4)))
